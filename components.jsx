...
function agentName(a, zh) { if (!a) return zh ? '未知智能体' : 'Unknown Agent'; return zh ? a.name : (a.enName || a.name); }
...
const {
  ...
  DEFECT_STREAM_PLANT_ID: _DEFECT_STREAM_PLANT_ID,
  INSPECTION_STREAM_PLANT_ID: _INSPECTION_STREAM_PLANT_ID,
...
const _MD_PURIFY_ATTR = ['target', 'rel', 'colspan', 'rowspan', 'style', 'class', 'width'];

function wrapMdTables(html) {
  return String(html ?? '')
...
function finalizeMarkdownHtml(html) {
  return wrapMdTables(html).replaceAll('<a ', '<a target="_blank" rel="noopener noreferrer" ');
}
...
function parseMarkdownHtml(md) {
  ...
  const clean = window.DOMPurify.sanitize(html, {
    ...
    ADD_ATTR: _MD_PURIFY_ATTR,
  });
  return finalizeMarkdownHtml(clean);
}
...
/** 为流式未闭合的 HTML 表格补上结束标签，便于浏览器按表格渲染 */
function closePartialHtmlTable(html) {
  let s = String(html ?? '');
  if (!/<table\b/i.test(s) || /<\/table>/i.test(s)) return s;
  const openTr = (s.match(/<tr\b/gi) || []).length;
  const closeTr = (s.match(/<\/tr>/gi) || []).length;
  for (let i = 0; i < openTr - closeTr; i++) s += '</tr>';
  return s + '</table>';
}

function parseStreamingBlockHtml(html) {
  const closed = closePartialHtmlTable(html);
  const parsed = parseMarkdownHtml(closed);
  if (parsed) return parsed;
  if (!window.DOMPurify) return null;
  const clean = window.DOMPurify.sanitize(closed, {
    USE_PROFILES: { html: true },
    ADD_ATTR: _MD_PURIFY_ATTR,
  });
  return finalizeMarkdownHtml(clean);
...
/**
 * 流式拆分：普通文本按「已完成行 + 当前行」；表格按整块解析，避免 <tr> 等以源码显示。
 */
function splitStreamingMarkdown(text) {
  const md = String(text ?? '');
  if (!md) return { stable: '', block: '', tail: '', mode: 'line' };

  const lower = md.toLowerCase();
  const tableIdx = lower.lastIndexOf('<table');
  if (tableIdx !== -1 && lower.indexOf('</table>', tableIdx) === -1) {
    return {
      stable: md.slice(0, tableIdx).trimEnd(),
      block: md.slice(tableIdx),
      tail: '',
      mode: 'html-table',
    };
  }
  ...
  let mdTableStart = lines.length;
  for (let i = lines.length - 1; i >= 0; i--) {
    if (isMdTableLine(lines[i])) mdTableStart = i;
    else if (mdTableStart < lines.length) break;
  }
  if (mdTableStart < lines.length) {
    return {
      stable: lines.slice(0, mdTableStart).join('\n'),
      block: lines.slice(mdTableStart).join('\n'),
      tail: '',
      mode: 'md-table',
    };
  }

  if (lines.length <= 1) return { stable: '', block: '', tail: md, mode: 'line' };
  return {
...
function Markdown({ text, streaming }) {
  ...
  if (!window.marked || !window.DOMPurify) {
    return <div className="md-body" style={{ whiteSpace: 'pre-wrap' }}>{md}</div>;
  }

  if (!streaming) {
    const html = parseMarkdownHtml(md);
    if (html === null) {
      return <div className="md-body" style={{ whiteSpace: 'pre-wrap' }}>{md}</div>;
    }
  ...
  const { stable, block, tail, mode } = splitStreamingMarkdown(md);
  const stableHtml = stable ? parseMarkdownHtml(stable) : '';
  const blockHtml = (mode === 'html-table' || mode === 'md-table') && block
    ? (mode === 'html-table' ? parseStreamingBlockHtml(block) : parseMarkdownHtml(block))
    : '';

  return (
    <div className="md-body md-streaming">
      {stableHtml ? <div dangerouslySetInnerHTML={{ __html: stableHtml }} /> : null}
      {blockHtml ? <div className="md-stream-block" dangerouslySetInnerHTML={{ __html: blockHtml }} /> : null}
      {mode === 'line' && tail ? <div className="md-stream-line">{tail}</div> : null}
    </div>
  );
}
...
/** 统一平移量：以全量末条的日历日对齐 today；全量/子集/异构列表共用，保证展示时间一致 */
function computeGlobalEventDeltaMs(now) {
  const today = startOfLocalDay(now);
  let lastTplTime = null;
  for (let i = _GET.length - 1; i >= 0; i--) {
    const t = resolveEventTime(_GET[i]);
    if (!isNaN(t.getTime())) {
      lastTplTime = t;
      break;
    }
  }
  if (!lastTplTime) return 0;
  return today.getTime() - startOfLocalDay(lastTplTime).getTime();
}
function buildGlobalEvents(now, templates = _GET) {
  const deltaMs = computeGlobalEventDeltaMs(now);
  return templates.map((tpl, i) => {
    const base = resolveEventTime(tpl);
    return {
      id: `global-${i}`,
      _idx: i,
      ...tpl,
      _time: new Date(base.getTime() + deltaMs),
    };
  });
}
...
function buildAgentWorkflowLog(agentId, now, zh, plantId) {
  const templates = getGlobalEventTemplatesForPlant(plantId);
  const events = buildGlobalEvents(now, templates);
  return events
    .map((e, i) => ({
      _idx: i,
      agent: e.agent,
      time: e._time,
      text: (!zh && e.en) ? e.en : e.text,
    }))
...
// 暗黑模式仅展示夜间时段 [18:00, 07:00] 的事件
function isNightEvent(evt) {
  const t = evt._time;
  if (!t) return true;
  const mins = t.getHours() * 60 + t.getMinutes();
  return mins >= 18 * 60 || mins <= 7 * 60;
}
function EventStream({onCollapse, focusPlant, theme}){
  ...
  const allEvents = React.useMemo(() => {
    const templates = getGlobalEventTemplatesForPlant(focusPlant?.id);
    const events = buildGlobalEvents(now, templates);
    const pid = focusPlant?.id != null ? String(focusPlant.id) : null;
    const scoped = (pid === _DEFECT_STREAM_PLANT_ID || pid === _INSPECTION_STREAM_PLANT_ID)
      ? events
      : events.filter(e => eventMatchesPlant(e, focusPlant));
    return theme === 'dark' ? scoped.filter(isNightEvent) : scoped;
  }, [now, focusPlant?.id, focusPlant?.name, theme]);
  ...
  return (
    <div className="panel stream corners"><span className="c1"/>
      <div className="panel-hd">
        <span><span className="dot"/> <T z="事件流" e="Event Stream"/></span>
...
const _WELCOME_MSG_ZH = '您好，iRun新能源运维数字团队随时待命，有任何问题请@我';
const _WELCOME_MSG_EN = 'Hello, the iRun New Energy Operations Digital Team is on standby. If you have any issues, please @me';
const _TEAM_BADGE = { code: 'iRun', cat: 'management' };

function isTeamWelcomeMsg(m) {
  return m?.role === 'team'
    && (m.text === _WELCOME_MSG_ZH || m.enText === _WELCOME_MSG_EN);
}

/** 欢迎语会话不在历史列表展示智能体标签 */
function sessionHistoryAgent(s) {
  const conversational = s.msgs.filter(m => m.role !== 'sys');
  if (conversational.length === 1 && isTeamWelcomeMsg(conversational[0])) return null;
  return s.agent || null;
}

const _SESSIONS_SEED = [
  { id:'s_cur', t:'当前', enT:'Current', dateLabel:'today', title:'当前会话', enTitle:'Current Session', isCurrent:true,
    msgs:[
      { role:'sys', text:_SESSIONS_SYS_ZH, enText:_SESSIONS_SYS_EN },
      { role:'team', text:_WELCOME_MSG_ZH, enText:_WELCOME_MSG_EN },
    ]},
...
function findExpoQAFuzzy(list, cleanText) {
  ...
  if (byKey.length > 1) {
    const scored = byKey.map(x => ({
      x,
      score: (x.matchKeys || []).filter(k => c.includes(normQaText(k))).length,
    }));
...
function pickExpoAnswer(hit, cleanText, zh, plant, rawText) {
  if (!hit) return '';
  if (hit.dynamic === 'healthiness') {
    const resolve = window.IRUN?.resolveHealthinessPlantKey;
    const build = window.IRUN?.buildHealthinessAnswer;
    if (resolve && build) {
      const plantKey = resolve(rawText || cleanText, plant);
      return build(plantKey, zh);
    }
  }
  if (hit.dynamic === 'health_advice') {
    const resolve = window.IRUN?.resolveHealthinessPlantKey;
    const build = window.IRUN?.buildHealthAdviceAnswer;
    if (resolve && build) {
      const plantKey = resolve(rawText || cleanText, plant);
      return build(plantKey, zh);
    }
  }
  if (hit.dynamic === 'alert_work_orders') {
    const build = window.IRUN?.buildAlertWorkOrderStatusAnswer;
    if (build) return build(zh);
  }
  if (hit.dynamic === 'alert_latest_decision') {
    const build = window.IRUN?.buildAlertLatestDecisionAnswer;
    if (build) return build(zh);
  }
  if (hit.dynamic === 'order_dispatch_first_alarm') {
    const build = window.IRUN?.buildOrderDispatchFirstAlarmAnswer;
    if (build) return build(zh);
  }
...
function resolveAgentReply(text, agentId, plant, zh) {
  ...
  if (hit) {
    return {
      stream: true,
      text: pickExpoAnswer(hit, cleanText, zh, plant, text),
    };
  }
...
function DispatchPanel({focusPlant, dispatchPlantCtx, selectedAgent, onSelectAgent, onClearDispatchPlantCtx, onOpenAgent, onCollapse, mode, onDispatchCommand, onDispatchToPlant, onRecallDispatch, theme='light'}){
  ...
  const dispatchBodyRef = useRef(null);
  ...
  const railCtx = { focusPlant, theme };
  ...
  function startStreamReply(sessionId, msgIndex, fullText) {
    ...
    (async () => {
      ...
      const applyUpdate = (text, streaming) => {
      ...
      const update = (text, streaming) => {
        if (!streaming) {
          ...
          applyUpdate(text, false);
          return;
        }
        const now = Date.now();
        if (!lastFlushAt || now - lastFlushAt >= UPDATE_MS) {
          if (throttleTimer) {
            clearTimeout(throttleTimer);
            throttleTimer = null;
          }
          applyUpdate(text, true);
          return;
        }
        if (throttleTimer) clearTimeout(throttleTimer);
        throttleTimer = setTimeout(() => {
          throttleTimer = null;
          applyUpdate(accumulated, true);
        }, UPDATE_MS);
      };
  ...
  function deleteSession(id, e){
    ...
    setSessions(prev => {
      ...
      if (next.length === 0){
        const fresh = { id:'s_cur', t:'当前', enT:'Current', dateLabel:'today',
  ...
  const prevChatScrollIdRef = useRef(currentId);

  const streamingScrollKey = React.useMemo(() => {
    const last = messages[messages.length - 1];
    if (last?.role === 'agent' && last.streaming) return msgText(last, zh);
    return '';
  }, [messages, zh]);
  ...
  const syncStickToBottom = () => {
    ...
    stickToBottomRef.current = dist < CHAT_SCROLL_STICK;
  };

  const scrollChatToBottom = (force) => {
    const el = bodyRef.current;
    if (!el) return;
  ...
  React.useLayoutEffect(() => {
    const forceSession = prevChatScrollIdRef.current !== currentId;
    prevChatScrollIdRef.current = currentId;
    if (forceSession) stickToBottomRef.current = true;
    const id = requestAnimationFrame(() => scrollChatToBottom(forceSession));
    return () => cancelAnimationFrame(id);
  }, [currentId, messages.length, streamingScrollKey]);

  // 底部快捷词/输入区高度变化 + 消息（含图片）撑高时，同步留白并滚到底
  React.useLayoutEffect(() => {
    const shell = dispatchBodyRef.current;
    const foot = footRef.current;
    const chat = bodyRef.current;
    if (!shell || !foot || typeof ResizeObserver === 'undefined') return;
    const apply = () => {
      shell.style.setProperty('--dispatch-foot-h', `${foot.offsetHeight}px`);
    ...
    ro.observe(foot);
    if (chat) {
      chat.querySelectorAll('.msg, .tag-line').forEach(el => ro.observe(el));
      chat.querySelectorAll('img').forEach(img => {
        if (!img.complete) img.addEventListener('load', apply, { once: true });
      });
    }
    return () => ro.disconnect();
  }, [currentId, selectedAgent, input, zh, messages.length, streamingScrollKey]);
  ...
  function newSession(){
    ...
    const fresh = {
      id, t: tLabel, enT: tLabel, dateLabel:'today',
  ...
  // 与 AgentsRail 同步：当前选中智能体被置灰时取消选中
  useEffect(() => {
    if (!selectedAgent || !isAgentRailDisabled(selectedAgent, railCtx)) return;
    onSelectAgent?.(null);
  }, [focusPlant?.id, theme, selectedAgent]);
  ...
  const send = (text, agentIdOverride) => {
    ...
    if (!targetId) return;
    if (isAgentRailDisabled(targetId, railCtx)) return;
  ...
  const quickChips = (() => {
    const enabled = _QP.filter(q => !isAgentRailDisabled(q.a, railCtx));
    return chipAgentId ? enabled.filter(x => x.a === chipAgentId) : enabled;
  })();
  ...
  const expoHit = effectiveAgent && ['pv'].includes(effectiveAgent.id)
    ...
    : null;
  const canSend = !!effectiveAgent && !isAgentRailDisabled(effectiveAgent.id, railCtx) && qaInput.length > 0 && (
  ...
  return (
    <div className={`panel dispatch corners${historyOpen?' history-open':''}`}><span className="c1"/>
      <div className="panel-hd">
            ...
            {['today','earlier'].map(group=>{
              ...
              return (
                <div key={group} className="hf-group">
                  ...
                  {items.map(s=>{
                    const histAgentId = sessionHistoryAgent(s);
                    const ag = histAgentId ? _ABI[histAgentId] : null;
                    const cat = ag && _CATS[ag.cat];
                    ...
                    return (
                      <div key={s.id}
                        ...
                        {ag && <span className="hf-tag" style={{color:cat?.color, borderColor:(cat?.color||'#666')+'55'}}>{ag.code}</span>}
      ...
      <div className="dispatch-body" ref={dispatchBodyRef}>
        <div className="chat-body" ref={bodyRef} onScroll={syncStickToBottom}>
          {messages.map((m,i)=>{
            ...
            const cat = isTeam ? _CATS[_TEAM_BADGE.cat] : (ag && _CATS[ag.cat]);
            const badgeCode = m.role==='user' ? 'YOU' : isTeam ? _TEAM_BADGE.code : ag?.code;
            const badgeColor = m.role==='user' ? 'var(--cyan)' : cat?.color;
            return (
              <div key={i} className={`msg ${m.role}`}>
                <div className="a" style={{borderColor:badgeColor, color:badgeColor}}>
                  {badgeCode}
                </div>
                <div className="b">
                  {!isTeam && (
                  ...
                  {m.role === 'agent' ? (
                    <>
                      <Markdown text={body} streaming={!!m.streaming} />
        ...
        <div className="dispatch-foot" ref={footRef}>
          <div className={`quick${quickChips.length >= 3 ? ' quick--2row' : ''}`}>
            {quickChips.map((q,i)=>{
              const ag = _ABI[q.a];
...
function respondTo(text, agentId, plant){
  ...
  if (agentId === 'query') {
    ...
    if (hit) return pickExpoAnswer(hit, cleanText, true, plant, text);
  }
  if (agentId === 'ops') {
    const cleanText = stripAgentPrefix(text);
  ...
  if (agentId === 'order') {
    ...
    if (hit) return pickExpoAnswer(hit, cleanText, normQaText(hit.qZh) === normQaText(cleanText), plant, text);
  }
  // PV Expo (2026) fixed dialogues for Alarm agent
  if (agentId === 'alert') {
    const cleanText = String(text || '').replace(/^@\S+\s*/, '').trim();
    const hit = findAlertQA(cleanText);
    if (hit) return pickExpoAnswer(hit, cleanText, normQaText(hit.qZh) === normQaText(cleanText), plant, text);
  }
...
function AgentModal({agentId, onClose, busyMap, onChat, focusPlantId}){
  ...
  const log = React.useMemo(
    () => buildAgentWorkflowLog(agentId, now, zh, focusPlantId),
    [agentId, now, zh, focusPlantId]
  );
  useEffect(() => {
...
//  · JOHOR (DEFECT_STREAM_PLANT_ID): 屋顶蛇形往返航线
//  · 其余电站: 椭圆航线（loop 时无限循环，否则 ×2 后飞出）
function DroneFlight({onDone, plant, loop=false}){
  const isJohor = plant?.id === _DEFECT_STREAM_PLANT_ID;
...
function EventStreamTab({onExpand, count=0}){
  const l = useLang(); const zh = l !== 'en';
  return (
    <div className="stream-tab" onClick={onExpand} title={zh?'展开事件流':'Expand Event Stream'}>
      <div className="stream-tab-inner">
        <span className="arrow">›</span>
        {zh ? (
          <>
            <span className="vlabel">事 件 流</span>
          </>
        ) : (
...
/** 与 AgentsRail 置灰规则一致：电站编制 / 演示站覆盖 / 暗色主题仅开放 ops·warn·safe */
function isAgentRailDisabled(agentId, { focusPlant, theme = 'light' } = {}) {
  const id = agentId;
  if (!id) return false;
  ...
  if (focusPlant) {
    const isOff = hasTeamOverride
      ? unavailableAgentIds.has(id)
      : !focusPlant.agents.includes(id);
    if (isOff) return true;
    if (demoProfile?.grayInspInTeam && id === 'insp') return true;
  }
  if (theme === 'dark' && !DARK_RAIL_AGENTS.has(id)) return true;
  return false;
}
    ...
    <div className="agents-rail" ref={railRef} onMouseLeave={()=>setHoverId(null)}>
      ...
      <div className="agents-rail-list">
        {_AGENTS.map(a=>{
          ...
          const disabled = isAgentRailDisabled(a.id, railCtx);
          const busy = busyMap?.[a.id];
          return (
            <div key={a.id}
